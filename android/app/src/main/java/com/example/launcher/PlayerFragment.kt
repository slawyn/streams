...
class PlayerFragment : DialogFragment() {
     ...
     *
     * The stream you provided appears to use a signed WMS-style
     * authentication query parameter, so we want ExoPlayer's HTTP
     * behavior to be as permissive as possible.
     */
    ...
    private fun setupPlayerListener(exo: ExoPlayer) {
        ...
        exo.addListener(object : Player.Listener {
            ...
            override fun onPlayerError(error: PlaybackException) {
                ...
                if (
                    causeMsg.contains("404", ignoreCase = true) ||
                    ...
                    android.util.Log.d(
                        TAG,
                        "Playback error detected, scheduling retry"
                    )
     ...
     *
     * This is especially important for this stream because we need
     * to distinguish:
     *
     * 403/401 -> signed URL/authentication problem
     * 404     -> wrong segment URL
     * parser  -> malformed/unusual HLS
     * decoder -> unsupported codec
     * timeout -> CDN/network issue
     */
        ...
        if (autoSpeedEnabled) {
            ...
            player?.let {
                ...
                if (
                    ...
                    it.setPlaybackParameters(
                        PlaybackParameters(targetSpeed)
                    )
                }
            }
        }
